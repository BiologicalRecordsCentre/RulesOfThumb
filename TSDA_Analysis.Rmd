...
This was done for all variables and the results from three of the variables are shown below:
...
* Third is the violin plot for the prop_list_one: the proportion of records from visits with list length of 1.

The `prop_repeats_grp` data shows a broad range of results from 0 to 1 for all taxonomic groups.  By contrast, the `prop_abs_list` data shows very clear division between taxonomic groups.  Similarly, the `prop_list_one` factor is distributed for some groups, but very close to 0 for others such as Lichens, FungusGnats and Rove Beetles.  Therefore, any partitioning on the basis of the `prop_abs_list` and `prop_list_one` variables are taxonomically biased.  For this reason, `prop_abs_list` and `prop_list_one` will not be used for producing decision trees.
...
```{r prop_list_one_violin,echo=FALSE,fig.cap="Violin plot for the prop_list_one variable, defined as the proportion of records from visits with list length equal to 1"}
#violin(td_taxa$prop_list_one,title='Violin Plot of proportion of list length one')
```
...
td_sub <- trendsData[,c('Spnvisits','P90','visits_P90','prop_of_years',
                           'prop_repeats_grp','prop_abs')]
pairs.panels(td_sub, hist.col = 'blue',smooth = TRUE)
```

```{r pairspanels.with.logs,fig.cap="Pairs plot for all variables used to construct the classification tree, plus the Spnvisits variable, defined as the total number of records for each species of interest.  Spnvisits, P90 and visits_P90 have been log transformed for this version."}
td_sub2 <- log10(trendsData[,c('Spnvisits','P90','visits_P90')]+1)
td_sub2 <- cbind (td_sub2, trendsData[,c('prop_of_years',
                           'prop_repeats_grp','prop_abs')] )
...
Overall, we found that `r round( mean(taxa_melt$value[taxa_melt$variable == "Removed"]), 2)` of all records were removed, varying from `r round( min(taxa_melt$value[taxa_melt$variable == "Removed"]), 2)` to `r round( max(taxa_melt$value[taxa_melt$variable == "Removed"]), 2)` across projects
...
Using the split in the models between 'good' and 'bad', you can create a classification tree that attempts to use the variables we have extracted to partition the data to the best of its ability using a decision tree.  Some of the species do not have the proportion of years converged.  For these we use the simple decision tree: mean_year_precision >= 87.
...
trendsData$bad <- as.factor(trendsData$bad)
...
fit <- rpart(bad ~ median + P90 + visits_median + visits_P90 +
               prop_of_years + prop_repeats_grp + prop_abs,
        method = "class",
        data = trendsData,
        parms = list(loss = matrix(c(0,goodweight,badweight,0),ncol = 2)),
        control = rpart.control(minsplit=nrow(trendsData)*0.05))
...
A graphic way of considering this is in the following graph:
...
```{r plot_decision_tree_by_taxon, fig.cap = "The results of the decision tree with P90 on a log-scale, and prop_abs on a logit scale, by taxonomic group", echo=FALSE}
...
badweight <- 1
goodweight <- 10
fit_good <- rpart(bad ~ median + P90 + visits_median + visits_P90 +
                    prop_of_years + prop_repeats_grp + prop_abs,
                  method = "class",
                  data = trendsData,
                  parms = list(loss = matrix(c(0,goodweight,badweight,0),ncol = 2)),
...
rpart.plot(pfit_good, extra = 108, under = F, type = 3, clip.right.labs = FALSE, branch=0.7)
```
...
goodweight <- 1
badweight <- 10
fit_bad <- rpart(bad ~ median + P90 + visits_median + visits_P90 +
                    prop_of_years + prop_repeats_grp + prop_abs,
             method = "class",
             data = trendsData,
             parms = list(loss = matrix(c(0,goodweight,badweight,0),ncol = 2)),
...
## A bit more investigation about the parameters in the decision tree
...
```{r plot.median.v.P90, echo=FALSE}

mod.M.v.P90 <- lm(log10(td$P90)~log10(td$median))

# re-order the dataset so that the data_good==bad is drawn last
td <- td[order(td$data_good),]
...
plot <- ggplot(data = td) +
  ...
  geom_hline(aes(yintercept = 8.65, 
                 linetype = 'P90 = 8.65'), colour = '#ff0000') +
  xlab(lab = 'Median') + ylab(lab = 'P90') +
  scale_x_log10() + scale_y_log10() + 
  labs(title = 'P90 vs Median, all data') +
  theme_bw()  +
  scale_color_manual(name = 'Model prediction', values = c('dark red','dark blue'),
                     guide = guide_legend(override.aes = 
                                            list(color = c('dark red',
                                                           'dark blue')))) +
  scale_linetype_manual(name = 'lines', values = c(2,1,1), 
                        guide = guide_legend(override.aes =
                                             list(color = c('#000000',
                                                            '#000000',
                                                            '#ff0000')))) #+
...
td.p90.8_7 <- subset(td, P90>8.7)

mod.limit.of.precision <- rq(
      log10(mean_year_precision) ~
        log10(P90), 
      tau = 0.05, data = td.p90.8_7)

# predict the points at P90 = 8.7 and max P90
segment.ys <- 
  10^(predict(mod.limit.of.precision,
              newdata = data.frame('P90' = c(8.7,max(td.p90.8_7$P90)))))
```
```{r plot_P90_by_precision, echo=FALSE}
plot <- ggplot() +
  geom_point(aes(x = td$P90,
                 y = td$mean_year_precision,
                 colour = td$data_good,
                 shape = td$model_good),
             alpha = '0.2') +
  ylab(lab = 'Mean Year Precision') + xlab(lab = 'P90') +
  scale_x_log10(labels = scales::comma) + scale_y_log10() + labs(title = 'P90 vs Mean Year Precision, all data') +
  scale_color_manual(name = 'Model prediction', values = c('dark red','dark blue'),
                     guide = guide_legend(override.aes = 
                                            list(color = c('dark red',
                                                           'dark blue')))) +
  scale_shape_manual(name = 'Model quality', values = c(4,16),
                     guide = guide_legend(override.aes = 
                                            list(shape = c(4,16)))) + 
  geom_hline(yintercept = 87) +
  geom_segment(aes(x = 8.7, y = segment.ys[1], 
               xend = max(td.p90.8_7$P90), yend = segment.ys[2]))

plot
```

This shows that after the cut-off (of P90 > 8.7), the more data there are (represented here by the increase in P90), the better the precision is bound to be. The line represents the quantile regression through the 5th percentile, so it shows that precision /can/ be high with low P90, but the lowest possible precision is guarenteed to be higher with higher P90, i.e. more data!
