...
There are some additional stats we can get from the raw data file. Again, these are for all groups not just CIRRUS/JASMIN runs. Given the memory requirements needed to reformat the plant and moth data I have removed them from this. We can work with them seperately by using the visit data files which are on JASMIN where they will have been created.
...
##now also want number of kmsq with any cover of each habitat
...
for(i in 1:length(quants)){
  for(j in 1:3){
    ##subset data in quant to quantile ranges (25%, 50%, 75%)
    sub_UK <- subset(UK_land, UK_land[,names(quants)[i]] > quants[j ,i])
    sub_UK_any_hab <- ifelse(sub_UK[,3:ncol(sub_UK)] > 0, 1, 0)
    quants_sums[j,i] <- sum(sub_UK_any_hab[,names(quants)[i]])
  }
}
```
