...
```{r rec_rem_hist_plot, fig.cap="Histogram showing number of records removed from each taxonomic group due to the input data requirement of repeat visits.  As can be seen, most taxonomic groups have less than 100,000 records removed due to this step, while a couple of groups have in excess of 400,000 removed.  These groups are shown below", echo = FALSE}
ggplot(data = df2, aes(RecordsExcluded)) + geom_histogram(binwidth = 2.5e4) +
  ylab('Count of taxonomic groups') + xlab('Number of records removed from group') +
...
The first node in this decision tree is `prop_abs` >= 0.987.  To meet this requirement, at least 98.7% of the data for the taxonomic group needs to come from species other than the species of interest.  This is equivalent to 76 additional records for other species for every 1 record of the species of interest.
...
```{r plot_decision_tree_by_taxon, fig.cap = "The results of the decision tree with P90 on a log-scale, and prop_abs on a logit scale, by taxonomic group and splitting 10 year and all year trends", echo=FALSE}

ggplot(data = trendsData, aes(x = prop_abs, y = P90, 
             col = as.factor(model_good))) +
  geom_point() +
  scale_x_continuous(breaks = c(.2,.999), trans = 'logit') +
  scale_y_continuous(breaks = c(10,1000), trans = 'log10') +
  scale_color_manual(values=c('#0571b035', '#ca002035'),
                     name='Trend output') + 
  facet_wrap(~Taxa)  +
  geom_segment(aes(x=0.2, y = 7.4, xend = 0.987, yend = 7.4), color = 'black') +
