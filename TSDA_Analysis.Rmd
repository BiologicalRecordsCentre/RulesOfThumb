...
There were `r length(unique(trendsDataData$Taxa_Root))` projects in the dataset.
There was an average of `r  median(table(trendsData$Taxa))` species per project, with a range of `r  min(table(trendsData$Taxa))` to `r  max(table(trendsData$Taxa))`
...
fit_fsdf.ConfMat <- table ( data.frame( predicted = predict(fit_fsdf, type = "vector"), score = fit_fsdf$y) )
accuracy.fit_fsdf <- sum(diag(fit_fsdf.ConfMat))/sum(fit_fsdf.ConfMat)
...
fit_fsdf2 <- rpart(good ~ mean_year_precision + precision_growth_rate + PropYrConverged,
                  method = 'class',
                  data = fsdf)

fit_fsdf2.ConfMat <- table ( data.frame( predicted = predict(fit_fsdf2, type = "vector"), score = fit_fsdf2$y) )
accuracy.fit_fsdf2 <- sum(diag(fit_fsdf2.ConfMat))/sum(fit_fsdf2.ConfMat)
...
* Using precision only:  accuracy is `r round(accuracy.fit_fsdf,3)` (Figure \@ref(fig:decisionsprecision))

* Using precision and convergences: accuracy is `r round(accuracy.fit_fsdf2,3)` (Figure \@ref(fig:plottreeconvergence))
...
td_sub2 <- log10(trendsData[,c('median','P90','visits_median','visits_P90')]+1)
td_sub2 <- cbind (td_sub2, trendsData[,c('prop_repeats_grp','prop_list_one','prop_abs','prop_abs_list')] )
...
```{r splitgoodbad}
trendsData$model_good <- rep('poor',nrow(trendsData))
trendsData$model_good[(trendsData$PropYrConverged < 0.3437 &
                  ...
                  (is.na(trendsData$PropYrConverged) &
                   trendsData$mean_year_precision >= 87)] <- 'acceptable'
trendsData$model_good <- factor(trendsData$model_good, levels = c('poor','acceptable'), ordered = TRUE)

# add a version for all 3 categories - see the 'High' Rmarkdown for full info
trendsData$model_good2 <- trendsData$model_good
trendsData$model_good2 <- factor(trendsData$model_good2, levels = c('poor','acceptable', 'high'), ordered = TRUE)
trendsData$model_good2[trendsData$mean_year_precision >= 2126] <- 'high'
...
All these decision trees can be plotted on the same graph (Figure \@ref(fig:plotdecisiontreesall)).  All data points below the lines are predicted to not produce a good model.  Those models coloured blue did produce a good model, those below did not.

```{r plotdecisiontreesall, echo=FALSE, fig.cap = "The results of the decision trees with P90 on a log-scale, and prop_abs on a logit scale"}
...
* Proportion of species for each group which can be modelled 
...
* Proportion of species for each group which pass the aspirational target (Figure \@ref(fig:RMasp))
...
```{r RMasp,fig.cap='Plot of number of species which can be modelled using the aspirational target.  As this is the aspirational target, the number of species which can be modelled is lower'}
...
get_num_spec_taxa_but <- function(data_file,hab_names,quants){
  ...
  count_num_spec <- function(hab){
    ...
    # Create a sumpresent species
    spp %>%
      lapply(function(x) if((hab_obs %>% filter(CONCEPT == x) %>% nrow())>0){1}) %>%
      unlist() %>%
      sum()
  }
  # Create a dataframe of counts of number of species in each habitat
...
for(rownum in as.numeric(row.names(hab_table_full[hab_table_full$value<0,]))){
  tax <- hab_table_full$taxa[rownum]
  hab <- hab_table_full$hab[rownum]
  hab_table_full$value[rownum] <- 0
  numspec <- hab_table_full$num_spec[rownum]
  good <- hab_table_full$value[hab_table_full$variable=='good' &
                                 hab_table_full$taxa == tax &
                                 hab_table_full$hab == hab]
  bad <- hab_table_full$value[hab_table_full$variable=='bad' &
                                 hab_table_full$taxa == tax &
                                 hab_table_full$hab == hab]
  hab_table_full$value[hab_table_full$variable=='absent' &
                         hab_table_full$taxa == tax &
                         hab_table_full$hab == hab] <- numspec - (good + bad)
  
}

write.csv(hab_table_full, 'hab_table_full.csv', row.names = FALSE)
```
...
# Calculate metrics for the regions
...
```{r writeallredmetrics,eval=FALSE}
...
write.csv(x = reg_table_full,'reg_table_full.csv',row.names=FALSE)
```
...
```{r fig2c,echo=FALSE,fig.cap="Histogram of the mean year precision for all models"}
ggplot(trendsData, aes(x = mean_year_precision)) + 
  geom_histogram(bins=40, fill = "grey50") +
  scale_x_log10(labels=scales::comma) + 
  theme_light() +
  geom_vline(xintercept = c(60,149), size = 1, linetype = 2) +
  geom_vline(xintercept = 2126, size = 1) +
  scale_y_continuous(expand = c(0,1)) +
  annotate("text", label = c("poor","acceptable","high"), x = c(10,200,1e4), y = 220, hjust = 0) +
  ylab("frequency")
  
```


```{r fig2a,echo=FALSE,fig.cap="Graph showing results of consulation with model experts, including the results of the decision tree.  Light grey points in the background show all models which were not part of the consultation"}
ggplot() +
  geom_point(data=trendsData[trendsData$mean_year_precision>6&
                               trendsData$mean_year_precision<1000000,],
             aes(x=PropYrConverged,
  ...
  xlab("Proportion of Years which Converged") + ylab("Mean year precision") +
  geom_segment(aes(x=0.34, y = 149, xend = 0, yend = 149), color = 'black') +
  geom_segment(aes(x=0.34, y = 60, xend = 1, yend = 60), color = 'black') +
  geom_segment(aes(x=0.34, y = 60, xend = 0.34, yend = 149), color = 'black') +
  geom_segment(aes(x=0, y = 87, xend = 1, yend = 87), color = 'black', linetype = 2) 
```
...
ggplot(data = trendsData %>%
         filter(mean_year_precision>5 & mean_year_precision<1e6 & mean_means>0.001)) +
  geom_point(aes(x = mean_year_precision, y = mean_means, color = model_good2),
             alpha = 0.1) +
  scale_color_manual(values = c('#d95f02','#7570b3','#1b9e77')) +
  ylab('Mean yearly occupancy') + xlab('Model yearly precision') +
  scale_y_log10(breaks = c(1,0.2,0.05,0.01)) +
  scale_x_log10(labels = comma) +
