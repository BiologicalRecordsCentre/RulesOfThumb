...
ev2 <- evtree(model_good_hi ~ median + P90 + visits_median + visits_P90 +
...
``` {r full.evtree, fig.cap = "Decision tree for rules of thumb, based on all metrics"}
plot(ev2)
```
...
TP2 <- sum(predict(ev2)==trendsData$model_good_hi & predict(ev2)=='good')
FP2 <- sum(predict(ev2)!=trendsData$model_good_hi & predict(ev2)=='good')
TN2 <- sum(predict(ev2)==trendsData$model_good_hi & predict(ev2)=='bad')
FN2 <- sum(predict(ev2)!=trendsData$model_good_hi & predict(ev2)=='bad')
...
The overall accuracy is `r round((TP + TN)*100 / (length(predict(ev))),1)`.

For the tree from all metrics:
Sensitivity is `r round(TN2 * 100/(TN2 + FN2),1)`%
Specificity is `r round(TN2 * 100/(TN2 + FP2),1)`%
Accuracy is `r round((TP2 + TN2) / (length(predict(ev2))),2)`
